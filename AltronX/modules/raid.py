    ...
    if e.sender_id in SUDO_USERS:
        ...
        if len(e.text) > 11:
            ...
            if int(user_id) in ALTRON:
                await e.reply("» у тσσ мвν ωαℓє н яє", parse_mode=None, link_preview=None)
            elif int(user_id) == OWNER_ID:
                await e.reply("» у тσσ мвν ωαℓє н яє", parse_mode=None, link_preview=None)
            elif int(user_id) in SUDO_USERS:
                await e.reply("» ¢нι ναι ρнℓє ∂єкн ℓє кση н ує ѕυ∂σ υѕєя н нмαяα...... αυкαтℓєѕѕ кι∂z", parse_mode=None, link_preview=None)
            else:
                que[user_id] = []
                gey = que.get(user_id)
                phucker = [user_id]
                gey.append(phucker)
                await e.reply("» нєнє αв яєρℓу кαяηє ρє тєяєкσ gαℓιуαη ρ∂єηgι !! ✅", parse_mode=None, link_preview=None)
            ...
            user_id = int(b.id)
            if int(user_id) in ALTRON:
                await e.reply("» αяє у тσσ мвν ωαℓє н", parse_mode=None, link_preview=None)
            elif int(user_id) == OWNER_ID:
                await e.reply("» αяє у тσσ мвν ωαℓє н", parse_mode=None, link_preview=None)
            elif int(user_id) in SUDO_USERS:
                await e.reply("» ¢нι ναι ρнℓє ∂єкн ℓє кση н ує ѕυ∂σ υѕєя н нмαяα...... αυкαтℓєѕѕ кι∂z", parse_mode=None, link_preview=None)
